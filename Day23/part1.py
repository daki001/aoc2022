...
def go_through_all_moves(elves, moves):
	for i in range(moves):
		elves = plan_moves(elves, i)
		#print_elves(elves)
	return elves


def calculate_square(elves, moves):
	elves = go_through_all_moves(elves, moves)
	print_elves(elves)
	x_min = min([x[0] for x in elves])
	x_max = max([x[0] for x in elves])
	y_min = min([x[1] for x in elves])
...
if __name__ == '__main__':
	...
	# print(file)
	print(calculate_square(file, 10))
