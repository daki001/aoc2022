...
class Monkey:
    ...
    def turn(self, modul):
        ...
        for element in self._items:
            self._counter += 1
            new_element = self._operation(element) % modul
...
def initialize_monkeys(monkeys, instructions):
    ...
    for line in instructions:
        ...
        if line.startswith('Monkey'):
            ...
            math *= monkeys[current_monkey]._condition
...
def rounds(number, monkeys, math):
    for i in range(number):
        for monkey in monkeys.keys():
            monkeys[monkey].turn(math)
...
if __name__ == '__main__':
    ...
    math = initialize_monkeys(monkeys, file)
    print(rounds(10000, monkeys, math))
