...
def calculate_covered_space(sensors, beacon, row):
	max_distance = 0
	for sens in sensors.keys():
		s = [int(sens.split(' ')[0]), int(sens.split(' ')[1])]
		max_distance = max(max_distance, abs(sensors[sens][0] - s[0]) + abs(sensors[sens][1] - s[1]))

	covered = list()
	for key in sensors.keys():
		s = [int(key.split(' ')[0]), int(key.split(' ')[1])]
		if row - max_distance <= abs(sensors[key][0] - s[0]) + abs(sensors[key][1] - s[1]) <= row + max_distance:
	...
	for i in covered:
		for el in beacon:
			if i[0] <= el[0] <= i[1] and el[1] == row and el not in used_beacons:
				count -= 1
				used_beacons.append(el)
		count += i[1] - i[0] + 1
	return count
...
def calculate_manhatten_distance(sensor, beacon, row):
	...
	#for i in range(distance + 1):
		if y_cord + (distance - i) == row:
			coords.append([x_cord - i, x_cord + i])
