...
def bruteforce(minutes, score, round_score, current: Cave, current2: Cave, max_value, used, next_cave, next_cave2, path1, path2, bs):
	if minutes < 0:
	...
	if round_score != max_value:
		best_score = score
		if next_cave is None and next_cave2 is None:
			paths = find_path(current, used)
			for goal, p in paths.items():
				used.append(goal)
				paths2 = find_path(current2, used)
				used.pop(len(used) - 1)
				for goal2, p2 in paths2:
					best_score = max(best_score, bruteforce(minutes - len(p) + 1, score + round_score * (len(p) - 1), round_score, p[0], p2[0], max_value, used.copy(), goal, goal2, p[1:], p2[1:], best_score))

			return best_score
		elif next_cave is None:

		elif next_cave2 is None:

		else:
			if current == next_cave:
				used.append(current)
				#score += round_score
				round_score += current.pressure
				#minutes -= 1
				next_cave = None
				return bruteforce(minutes, score, round_score, current, max_value, used.copy(), None, bs)
			if current2 == next_cave2:
				used.append(current2)
				#score += round_score
				round_score += current2.pressure
				#minutes -= 1
				next_cave2 = None

			return bruteforce(minutes, score, round_score, current, max_value, used.copy(), next_cave, next_cave2, bs)
