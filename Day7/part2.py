...
def calculate_simple_sizes(tree):
	sum = 0
	for key,value in tree.items():
		if not isinstance(value, int):
			#print(value)
			subdirectory = calculate_simple_sizes(value)
			sum += subdirectory
		else:
			#print(value)
			sum += value
...
def calculate_sizes(tree, search_value):
	sum = 0
	directories = list()
	for key,value in tree.items():
		if not isinstance(value, int):
			#print(value)
			subdirectory = calculate_sizes(value, search_value)
			if subdirectory[0] >= free_space:
...
if __name__ == '__main__':
	...
	#print(tree)
	free_space = 30000000 - (70000000 - calculate_simple_sizes(tree))
	#print(free_space)
	print(min(calculate_sizes(tree, free_space)[1]))
